...
MyLDDT::Result MyLDDT::GetScore(MODE mode) {
	...
	for (auto &mod_res : Mod_sub_flip.residues) {
		...
		for (auto &flippair : mod_res.Flip(0)) {
			...
			lddt_orig += std::get<0>(lddt(A1, Mod_sub_fixed, *A1ref, Ref_sub_fixed));
			lddt_orig += std::get<0>(lddt(A2, Mod_sub_fixed, *A2ref, Ref_sub_fixed));

			// score flipped atoms
			lddt_flip += std::get<0>(lddt(A1, Mod_sub_fixed, *A2ref, Ref_sub_fixed));
			lddt_flip += std::get<0>(lddt(A2, Mod_sub_fixed, *A1ref, Ref_sub_fixed));
		}
	...
	// aggregate atomic scores into residue and total scores
	double stotal = 0.0;
	for (unsigned i = 0; i < Mod.nRes; i++) {
		const Residue &R = Mod.residues[i];
		double rtotal = 0.0;
		for (auto &A : R.atoms) {
			Atom *Aref = Ref_sub.GetAtom(A);
			std::tuple<double,int,int> s;
			if (Aref != NULL) {
				s = lddt(A, Mod_sub_flip, *Aref, Ref);
				result.amask[A.atomNum] = 1;
			}
			result.ascore[A.atomNum] = std::get<0>(s);
			result.rscore[i] += std::get<1>(s);
			rtotal += std::get<2>(s);
		}
		result.score += result.rscore[i];
		stotal += rtotal;
		result.rscore[i] = rtotal > 0.0 ? result.rscore[i] / rtotal : 0.0;
	}

	result.score = stotal > 0.0 ? result.score / stotal : 0.0;
...
std::tuple<double,int,int> MyLDDT::lddt(const Atom &Amod, const Chain &Mod_, const Atom &Aref, const Chain &Ref_) {
	...
	*/
	...
	nmax *= dbins.size();
	if (nmax > 0) {
		score = 1.0 * ngood / nmax;
	}

	return {score, ngood, nmax};
